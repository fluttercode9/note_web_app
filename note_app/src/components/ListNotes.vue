...
export default {
  ...
  async mounted() {
    ...
    const recordingListRef = ref(
      storage,
      `users/${auth.currentUser.uid}/recordings`
    );
    ...
    recordingStorageSnapshot.items.forEach((itemRef) => {
      getDownloadURL(ref(storage, itemRef._location.path_)).then((file_url) => {
        ...
        this.recording_notes.push(file_object);
            ...
            console.log(this.recording_notes, 'here')
  ...
  <div class="mx-auto">
    ...
    <div class="container-flex" v-if="photo_notes">
      <TransitionGroup name="list">
        <div
          class="card mb-1"
          v-for="photo_note in photo_notes"
          :key="photo_note.title"
        >
          <div
            class="card-body"
            style="text-align: justify; text-justify: inter-word"
    ...
    <div class="container-flex" v-if="recording_notes">
      <TransitionGroup name="list">
        <div
          class="card mb-1"
          v-for="recording_note in recording_notes"
          :key="recording_note.title"
        >
